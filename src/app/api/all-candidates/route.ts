...
export async function GET(request: Request) {
  try {
    ...
    // Summary KPIs (scoped to country if selected, otherwise full)
    ...
    const getCountrySpecificStages = (cName: string) => {
      ...
      if (norm.includes("romania") || norm.includes("italy") || norm.includes("poland")) {
        return [
          "Passport Entry",
          "Medical",
          "Police Clearance",
      ...
      // General Overseas (Bahrain, Oman, Qatar, Kuwait, Malaysia, Singapore, etc.)
      return [
        "Passport Entry",
        "Medical",
        "Police Clearance",
